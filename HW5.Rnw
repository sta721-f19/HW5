...
\item  If $\tau = 1/g \sim G(1/2, n/2)$, derive the conditional posterior distribution of $\tau \mid \beta_0, \beta, \phi, \Y$.  {\em Use the likelihood decomposition above}

\item  Using the Prostate data from HW4 and the model {\tt lcavol $\sim$ 1 + factor(Gleason)}, use JAGS and an approximation to the independent Jeffreys prior $p(\beta_0, \phi) \propto 1/\phi$, to  obtain 95\% credible intervals for the 4 group means using MCMC based on your distributions above under
\begin{enumerate}
  \item  the  g-prior above for $\b \mid \phi, \beta_0$ (with centered $\X$) with $g=n$

  \item  mixtures of the g-prior above where $\tau = 1/g \sim G(1/2, n/2)$

\item    will your results be the same if you change the baseline category?  Explain.
